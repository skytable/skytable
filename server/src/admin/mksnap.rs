    ...
    fn mksnap(handle: &crate::corestore::Corestore, con: &mut T, mut act: ActionIter<'a>) {
        ...
        if act.is_empty() {
            ...
            match engine.mksnap(handle.clone_store()).await {
                SnapshotActionResult::Ok => con._write_raw(groups::OKAY).await?,
                SnapshotActionResult::Failure => return util::err(groups::SERVER_ERR),
                SnapshotActionResult::Disabled => return util::err(groups::SNAPSHOT_DISABLED),
                SnapshotActionResult::Busy => return util::err(groups::SNAPSHOT_BUSY),
                _ => unsafe { impossible!() },
            }
        } else if act.len() == 1 {
            ...
            match engine.mkrsnap(name, handle.clone_store()).await {
                ...
                SnapshotActionResult::AlreadyExists => {
                    return util::err(groups::SNAPSHOT_DUPLICATE)
                }
                _ => unsafe { impossible!() },
            }
        } else {
            return util::err(groups::ACTION_ERR);
        }
        Ok(())
    }
);
